-- ZevDash_Engine.lua
-- BaseModule and registerClass must be defined at load time (not inside an event handler)
-- because class files execute at load time and call registerClass() immediately.
-- These functions only reference `snd` at call time, so they don't need `snd` to exist yet.
...
-- Class change handler: save old profile, load new profile, refresh display
-- This runs at runtime (when gmcp.Char.Status updates), so ZevDash.saveState etc.
-- will be available by then even though they're defined by other scripts.
ZevDash._lastClass = ZevDash._lastClass or (ZevDash.getCurrentClass and ZevDash.getCurrentClass()) or "unknown"

if not ZevDash._classChangeHandler then
    ZevDash._classChangeHandler = registerAnonymousEventHandler("gmcp.Char.Status", function()
        if not snd then return end
        local newClass = ZevDash.getCurrentClass()
        local oldClass = ZevDash._lastClass

        if newClass ~= oldClass and newClass ~= "unknown" then
            -- Save the old class's defense profile
            if oldClass and oldClass ~= "unknown" and ZevDash.saveState then
                ZevDash.saveState()
            end

            -- Load the new class's defense profile
            if ZevDash.loadState then
                ZevDash.loadState()
            end

            -- Apply def locks for the new class
            if ZevDash.applyDefLocks then
                ZevDash.applyDefLocks()
            end

            -- Refresh the dashboard if it's visible
            if ZevDash.is_built and ZevDash.visible then
                ZevDash.displayPage(ZevDash.active_page or "class")
            end

            ZevDash._lastClass = newClass
        end
    end)
end
